"""
CPBV 임팩트 투수 정보 자동 추출기
--------------------------------------
게임에서 투수 카드 화면을 열어두면 자동으로:
  - P1: 이름, 오버롤, 포지션, 팀, 능력치 6종 추출 (임팩트용 이름 정제)
  - P2: 잠재력 5종 (장타억제, 속구구종, 침착성, 변화구구종, 견제) 1~4 값
  - P3: 체력바 구간(픽셀), 구종 + 등급(S/A/B/C/D/E) 추출

사용법:
  python -m games.cpbv.classifier.extract_impact_pitcher
  python -m games.cpbv.classifier.extract_impact_pitcher --count 30
  python -m games.cpbv.classifier.extract_impact_pitcher --out D:/data/pitchers.json
...
def clean_player_name(name_raw: str) -> str:
    if not name_raw:
        return ""
    # 1. 완전히 기존 extract_pitcher.py의 로직 수행
    ...
    # - 공백이 있으면 첫 번째 토큰만 가져옴 (예: "후라도 전문 기계" -> "후라도")
    tokens = nc.split()
    if tokens:
        nc = tokens[0]
        
    # - 노이즈 접미사 제거 (예: "알칸타라그러나" -> "알칸타라")
    noisy_suffixes = ['기계', '기술', '기준도', '기준', '기념', '기년', '기록', '기획', '기재', '구조', '고객', '그러나']
    for suffix in noisy_suffixes:
        if nc.endswith(suffix):
            rem = nc[:-len(suffix)].strip()
            if len(rem) >= 2:
                nc = rem
...
def extract_pitcher_p1(frame, regions=None):
    ...
    # 1) 이름 정제 (임팩트 전용: 한글 이름 + 동명이인 구분용 영어 접미사 추출)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--count',       default=0,           type=int,
                        help='추출할 선수 수 (0=무한)')
    parser.add_argument('--name',        default='impact', help='카드 종류 이름 (출력 파일명 접두사)')
    ...
    if not args.out:
        if args.name == 'impact':
            filename = f'impact_pitchers_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
        elif args.name.startswith('impact_'):
            filename = f'{args.name}_pitchers_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
        else:
            filename = f'impact_{args.name}_pitchers_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
        args.out = os.path.join(_HERE, '..', '..', '..', 'data', filename)
